...
func InitServiceContext(config map[string]any) {
	once.Do(func() {
		...
		// 持久层配置
		dbLocation := configService.GetOrDefaultFromStartupConfig(consts.OrmDbLocation, "./db/app.db")
...
func (s *ServiceContext) StartReceiveServer() {
	port := s.ConfigService.GetOrDefault(consts.TransServerPort, consts.DefaultTransmitPort)
